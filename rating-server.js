...
const { app: gplayApp } = require("google-play-scraper");
const { app: appstoreApp } = require("app-store-scraper");
...
app.get("/api/play-store/:appId", async (req, res) => {
  ...
  try {
    ...
    const appData = await retryFetch(() =>
      gplayApp({ appId, lang: "en", country: "fr" })
    );
    console.log(appData);
...
app.get("/api/app-store/:appId", async (req, res) => {
  ...
  try {
    ...
    const appData = await retryFetch(() =>
      appstoreApp({ id: appId, lang: "en", country: "fr" })
    );
    console.log(appData);
